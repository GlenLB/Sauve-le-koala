...
class BrickClass extends Sprite {
    ...
    constructor(nomImage: string) {
        ...
        if (!Device.mobile) {
            this.width = 80;
            this.height = 40;
            BrickClass.padding = 10;
            BrickClass.offsetY = 20;
            BrickClass.offsetX = 25;
        } else {
            this.width = 40;
            this.height = 20;
            BrickClass.padding = 5;
            BrickClass.offsetY = 10;
            BrickClass.offsetX = 12.5;
        }
    }
    ...
    static initArray() {
        ...
        for (let c = 0; c < BrickClass.nbColumn; c++) {
            ...
            for (let r = 0; r < BrickClass.nbRow; r++) {
                if (r >= 1 && r <= 4 && c >= 1 && c <= 3) { //=> ironBrick
                    BrickClass.array[c][r] = {x: 0, y: 0, lives: 3, number: i};
                    i++;
                } else { // => Brick
    ...
    static drawBricks() {
        for (let c = 0; c < BrickClass.nbColumn; c++) {
            for (let r = 0; r < BrickClass.nbRow; r++) {
                ...
                if (r >= 2 && r <= 3 && c >= 2 && c <= 2) { // Space for the cat
                    BrickClass.catSpace = true;
                }
                if (b.lives > 0 && !BrickClass.catSpace) {
                    if (r >= 1 && r <= 4 && c >= 1 && c <= 3) { // ironBrick
                        b.x = (c * (IronBrick.width + BrickClass.padding)) + BrickClass.offsetX;
                        b.y = (r * (IronBrick.height + BrickClass.padding)) + BrickClass.offsetY;
                        ...
                        if (b.lives === 3) {
                            ...
                            Canvas.ctx.drawImage(IronBrickCrackedAgain.elt, b.x, b.y);
                        }
                    } else { // brick
                        ...
                        if (b.lives == 2) {
                            ...
                            Canvas.ctx.drawImage(BrickCracked.elt, b.x, b.y);
                        }
                    }
                }
            }
...
class PlayerClass {
    ...
    win() {
        ...
        let includes17: boolean = BrickClass.isInclude(17);
        let includes20: boolean = BrickClass.isInclude(20);
        let includes25: boolean = BrickClass.isInclude(25);
        let includes26: boolean = BrickClass.isInclude(26);
        ...
        if (// Première colonne
            ...
            // Première ligne
            || (includes9 && includes17)
            || (includes17 && includes25)
            // Deuxième ligne
            || (includes12 && includes20)
            || (includes20 && includes28)
            // Deuxième colonne
            || (includes25 && includes26)
            || (includes26 && includes27)
    ...
    drawWin() {
        ...
        this.buttonWin.innerHTML = "Bravo tu as complété tous les niveaux !<br>Le Koala est maintenant sauvé !";
    }
...
let IronBrickCrackedAgain: BrickClass = new BrickClass("iron_brick3");
