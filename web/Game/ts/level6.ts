...
class Sprite {
    ...
    static AllLoaded(): boolean {
        return Brick.load && BrickCracked.load && BlueBrick.load && BlueBrickCracked.load && GreenBrick.load && GreenBrickCracked.load && IronBrick.load && IronBrickCracked.load && Paddle.load && Koala.load
    }
}
        ...
        for (let c = 0; c < BrickClass.nbColumn; c++) {
            for (let r = 0; r < BrickClass.nbRow; r++) {
                ...
                if (b.lives > 0 && !BrickClass.catSpace) {
                    ...
                    } else if (c >= 2 && r <= 3) { //=> GreenBrick
                        if (b.lives === 2) {
                            Canvas.ctx.drawImage(GreenBrick.elt, b.x, b.y);
                        } else if (b.lives === 1) {
                            Canvas.ctx.drawImage(GreenBrickCracked.elt, b.x, b.y);
                        }
                    } else if ((c == 0) || (r == 8)) { //=> IronBrick
                        if (b.lives === 2) {
                            Canvas.ctx.drawImage(IronBrick.elt, b.x, b.y);
                        } else if (b.lives === 1) {
                            Canvas.ctx.drawImage(IronBrickCracked.elt, b.x, b.y);
                        }
                    } else { // brick
...
class PlayerClass {
    ...
    drawWin() {
        ...
        this.buttonWin.innerText = "Aller au niveau 7";
        this.buttonWin.addEventListener("click", () => {
            window.location.href = "http://" + window.location.host + "/sauve-le-koala?level=7&scroll=" + window.scrollY;
        });
    }
...
let IronBrickCracked: BrickClass = new BrickClass("iron_brick2");
