...
class Sprite {
    ...
    static AllLoaded(): boolean {
        return Brick.load && BrickCracked.load && BlueBrick.load && BlueBrickCracked.load && VioletBrick.load && VioletBrickCracked.load && Paddle.load && Koala.load
    }
}
...
class BrickClass extends Sprite {
    ...
    constructor(nomImage: string) {
        ...
        if (!Device.mobile) {
            ...
            BrickClass.offsetY = 8;
            BrickClass.offsetX = 58;
        } else {
            this.width = 40;
            this.height = 20;
            BrickClass.padding = 4;
            BrickClass.offsetY = 4;
            BrickClass.offsetX = 29;
        }
    }
    ...
    static drawBricks() {
        for (let c = 0; c < BrickClass.nbColumn; c++) {
            for (let r = 0; r < BrickClass.nbRow; r++) {
                ...
                if ((r == 0 && c == 4) || (r == 1 && c == 4)) { // Space for the cat
                ...
                if (b.lives > 0 && !BrickClass.catSpace) {
                    if (c >= 3 && r <= 2) { //=> BlueBrick
                        if (b.lives === 2) {
                            Canvas.ctx.drawImage(BlueBrick.elt, b.x, b.y);
                        } else if (b.lives === 1) {
                            Canvas.ctx.drawImage(BlueBrickCracked.elt, b.x, b.y);
                        }
                    } else if (c >= 2 && r <= 3) { //=> VioletBrick
                        if (b.lives === 2) {
                            Canvas.ctx.drawImage(VioletBrick.elt, b.x, b.y);
                        } else if (b.lives === 1) {
                            Canvas.ctx.drawImage(VioletBrickCracked.elt, b.x, b.y);
                        }
                    } else { // brick
...
class PlayerClass {
     ...
     */
    win() {
        let includes27: boolean = BrickClass.isInclude(27);
        let includes28: boolean = BrickClass.isInclude(28);
        let includes29: boolean = BrickClass.isInclude(29);
        let includes38: boolean = BrickClass.isInclude(38);

        if (
            (includes27 && includes28) ||
            (includes28 && includes29) ||
            (includes29 && includes38)
        ) {
            this.hasWon = true;
            if (AudioClass.soundOK) {
                WinLevelSound.play();
    ...
    drawWin() {
        ...
        this.buttonWin.innerText = "Aller au niveau 6";
        this.buttonWin.addEventListener("click", () => {
            window.location.href = "http://" + window.location.host + "/sauve-le-koala?level=6&scroll=" + window.scrollY;
        });
    }
...
let VioletBrick: BrickClass = new BrickClass("brick_violet");
let VioletBrickCracked: BrickClass = new BrickClass("brick_violet_cracked");
let BlueBrick: BrickClass = new BrickClass("brick_blue");
let BlueBrickCracked: BrickClass = new BrickClass("brick_blue_cracked");
BrickClass.initArray();

// Koala
let Koala: Sprite = new Sprite("koala");
Koala.x = (4 * (Brick.width + BrickClass.padding)) + BrickClass.offsetX;
Koala.y = (0 * (Brick.height + BrickClass.padding)) + BrickClass.offsetY;
